const extractData = (text) => {
    ...
    const skillsRegex = /(react|node\.?js|javascript|python|java|mongodb|express|tailwind|html|css|sql|git|aws|docker|typescript|next\.?js|flutter|angular|redux|firebase|mysql|postgresql)/gi;
    ...
    return {
        ...
        experienceLevel: text.includes("year") || text.includes("yrs") || text.includes("experience") ? "Experienced" : "Fresher",
    };
};
...
const calculateScore = (resumeText, jobDesc) => {
    ...
    const keywordMatch = jobWords.length ? Math.round((matchCount / jobWords.length) * 100) : 50;
    let atsScore = Math.min(95, keywordMatch + 28);

    if (resumeText.includes("experience") || resumeText.includes("education")) atsScore += 7;
...
// New Function: Get Matched & Missing Skills
const getSkillsAnalysis = (resumeText, jobDesc) => {
    const commonSkills = ["react", "node", "javascript", "mongodb", "express", "sql", "git", "html", "css", "python", "java", "aws", "docker"];
...
module.exports = { 
    ...
    calculateScore, 
    getSuggestions,
    getSkillsAnalysis   // ← Exporting new function
};